...
from log_formatter import log_result
...
# um único valor.
    ...
    log_result(1, "Transações contendo Brasil (RDD)", count_brasil_transactions_RDD)
...
# ser composto de chave (Brazil) e valor.
    ...
    log_result(2, "Transações contendo Brasil (PairRDD)", count_brasil_transactions_PairRDD.collect())
...
# retorno deverá ser um único valor
    ...
    log_result(3, "Transações contendo Brasil en 2016 (RDD)", count_brasil_transactions_2016_RDD)
...
# retorno deverá ser com chave (Brasil, 2016) e valor.
    ...
    count_brasil_transactions_2016_PairRDD = data.filter(lambda transaction: transaction[0] == "Brazil" and transaction[1] == "2016") \
                       ...
                       .aggregateByKey(0, lambda total_count, current_count: total_count + current_count, lambda partition1, partition2: partition1 + partition2)
    log_result(4, "Transações contendo Brasil em 2016 (PairRDD)", count_brasil_transactions_2016_PairRDD.collect())
...
# flow) e valor.
    ...
    log_result(5, "Número de transações por flow e por ano (PairRDD)", count_transactions_year_flow_list)
...
# conter apenas o valor da média.
    ...
    log_result(6, "Média da coluna Price para o ano de 2016 (RDD)", avg_price_2016_RDD)
...
# conter chave (ano) e valor.
    ...
    log_result(7, "Média da coluna Price para o ano de 2016 (PairRDD)", avg_price_2016_PairRDD.collect())
...
# (Flow=Export). A saída esperada é chave(país, flow) e valor.
    ...
    log_result(9, "País com valor máximo de exportação (PairRDD)", max_export)
...
# (1,0 ponto) O preço mínimo por país e por ano, ordenado por ano. Utilizar PairRDD.
    ...
    log_result(10, "Preço mínimo por país e ano (PairRDD)", min_price_sorted)
